...
namespace Nurbsy.Helpers
{
    public static class NurbsSurface2DHelper
    {
        ...
        public static NurbsCurve<Vector2> GetIsoCurveU(in NurbsSurface<Vector2> surface, float u)
        {
            ...
            var isoCps = new ControlPoint<Vector2>[countV];
            ...
            for (int j = 0; j < countV; j++)
            {
                ...
                isoCps[j] = w != 0
                    ? new ControlPoint<Vector2>(weighted / (float)w, w)
                    : new ControlPoint<Vector2>(Vector2.Zero, 0.0);
            }

            return new NurbsCurve<Vector2>(surface.DegreeV, isoCps, surface.KnotsV);
        }

        /// <inheritdoc cref="NurbsSurface{T}.GetCurveV(float)"/>
        public static NurbsCurve<Vector2> GetIsoCurveV(in NurbsSurface<Vector2> surface, float v)
        {
            ...
            var isoCps = new ControlPoint<Vector2>[countU];
            ...
            return new NurbsCurve<Vector2>(surface.DegreeU, isoCps, surface.KnotsU);
        }
        ...
        )
        {
            ...
            // so leaving the corresponding entries in `derivatives` at Vector2.Zero is correct.
