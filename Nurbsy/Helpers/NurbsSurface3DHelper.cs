...
namespace Nurbsy.Helpers
{
    public static class NurbsSurface3DHelper
    {
        ...
        )
        {
            ...
            var tempPatches = new ControlPoint<Vector3>[tempPatchCount][][];
            for (int i = 0; i < tempPatchCount; i++)
            {
                tempPatches[i] = new ControlPoint<Vector3>[degreeU + 1][];
                for (int j = 0; j <= degreeU; j++)
                {
                    tempPatches[i][j] = new ControlPoint<Vector3>[columns];
                }
            }
            ...
            var bezierPatches = new List<ControlPoint<Vector3>[][]>(finalPatchCount);

            for (int i = 0; i < finalPatchCount; i++)
            {
                var patch = new ControlPoint<Vector3>[degreeU + 1][];
                for (int j = 0; j <= degreeU; j++)
                {
                    patch[j] = new ControlPoint<Vector3>[degreeV + 1];
                }
            ...
            var result = new List<BezierSurface<Vector3>>(nb);
            for (int i = 0; i < nb; i++)
            {
                result.Add(new BezierSurface<Vector3>(degreeU, degreeV, bezierPatches[i]));
            }
        ...
        )
        {
            ...
            if (isUDirection)
            {
                ...
                for (int i = 0; i < transposed.Count; i++)
                {
                    ...
                    if (!NurbsCurve3DHelper.RemoveKnot(curve, removeKnot, times, out var removed))
                    {
                        result = surface;
                        return false;
                    }

                    var cpArray = new ControlPoint<Vector3>[removed.ControlPoints.Count];
        ...
        public static void EquallyTessellate(
            in NurbsSurface<Vector3> surface,
            out IReadOnlyList<Vector3> tessellatedPoints,
        ...
        )
        {
            ...
            var lastPoint = MathUtils.IsZero(lastCP.Weight) ? Vector3.Zero : lastCP.Value;
        ...
        )
        {
            ...
            for (int iter = 0; iter < maxIterations; iter++)
            {
                ...
                double fa = Vector3.Dot(Su, difference);
                double fb = Vector3.Dot(Sv, difference);
                ...
                double gu = Vector3.Dot(Su, Sv) + Vector3.Dot(difference, Suv);
                double gv = Vector3.Dot(Sv, Sv) + Vector3.Dot(difference, Svv);
        ...
        public static NurbsSurface<Vector3> Reparametrize(
            in NurbsSurface<Vector3> surface,
            double minU,
            double maxU,
            double minV,
        ...
        )
        {
            ...
            // First fundamental form coefficients
            double a = Vector3.Dot(Su, Su); // E
            double b = Vector3.Dot(Su, Sv); // F
            double c = Vector3.Dot(Su, Sv); // F (same as b)
            double d = Vector3.Dot(Sv, Sv); // G

            // Project tangent onto surface derivatives
            double e = Vector3.Dot(Su, tangent);
            double f = Vector3.Dot(Sv, tangent);
        ...
        public static bool IsClosed(in NurbsSurface<Vector3> surface, SurfaceDirection direction)
        {
            ...
            if (isUDirection)
            {
                ...
                for (int i = 0; i < transposed.Count; i++)
                {
                    ...
                    if (!NurbsCurve3DHelper.IsClosed(curve))
                    {
                        return false;
                    }
                }
